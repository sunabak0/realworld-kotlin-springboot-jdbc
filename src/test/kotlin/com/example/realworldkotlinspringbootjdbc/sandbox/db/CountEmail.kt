...
/**
 * @TestInnstance(TestInstance.Lifecycle.PER_CLASS)
 *
 * 参考: KotlinとJUnit5とTestInstanceアノテーション
 * https://footaku.hatenablog.com/entry/2018/07/10/140000
 */
...
class CountEmail {
    ...
    fun SQLのcountを試す() {
        val namedParameterJdbcTemplate = DbConnection.namedParameterJdbcTemplate
        val sql = "SELECT count(email) FROM users WHERE users.email = :email"
    ...
    fun SQLのcountを試す2() {
        ...
        val sqlParams = MapSqlParameterSource()
            .addValue("email", "dummy@example.com")
        val actual = namedParameterJdbcTemplate.queryForMap(sql, sqlParams)
        val expected = mapOf("count" to 1L)
        assertThat(actual).isEqualTo(expected)
    }
}
